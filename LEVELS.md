# Pizza Palace: Level 1-30 Design Plan
...
| 17 | **Percent** | the same split written "out of 100"; graded from level 16 on the 8- and 10-slice only, where the percent is a whole number (each 10-slice slice = 10%, each 8-slice quarter = 25%); no inline answer-hint in the order text (the method is taught by the clickable glossary card); concept `percent`, glossary card `percent` |
| 18 | **Decimal** | the same split written with a dot, another name for a part the child can build as a fraction; graded from level 15 on the 8- and 10-slice only, where the decimal lands on whole slices (0.1 = one slice on a ten-slice, 0.25 = one quarter on an eight-slice); the order poses the decimal only, the glossary card teaches the method (no in-order hint); concept `decimal`, glossary card `decimal` |
| 19 | **Cross-denominator** | the two-pizza order at level 26+ becomes two TWELVE-slice pizzas (a 24-slice pool), the only board where thirds (8/24), quarters (6/24), sixths (4/24), eighths (3/24) and twelfths (2/24) all land on whole slices and can share ONE order (a third beside an eighth, impossible on the 16-slice pool). Reuses the Mode B pool grader at `boardN: 12`; the fraction word is the problem, no inline hint; glossary cards `third`/`eighth`/`twelfth` etc. |
...
- **Granularity**: whole (1) -> halves (2-3) -> three-region (4) -> quarters (5-6) -> pairs/slices (7-8) -> sequences (13, 18) -> full composite (17-25). Rises, never regresses. The old 17->20 cram is stretched into a smooth 17->25 climb; the densest pool (the old Level 20) now lands at Level 25. Levels 26-30 hold that single-board composite peak (28-30 add ANGLE wedges, a new concept, as the one lighter draw; the bare thirds/12-slice fillers were removed so the composites and heavy mutators dominate the top) and add the CROSS-DENOMINATOR two-pizza pool (two 12-slice pizzas = 24 slices, thirds and eighths in one order); the climb slows from 3 tipped wins in a row to leave Level 25 up to 7 to leave Level 29.
